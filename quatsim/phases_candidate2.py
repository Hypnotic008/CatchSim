...
class Segment:
    ...
    # between 8 and 533 m.
    max_tilt: float = np.radians(30.0)
    ...
    def _actuate(self, seg: Segment, s: np.ndarray, t_seg: float,
        ...
        if seg.mode == Mode.LANDING:
            ...
            if terminal_phase and seg.terminal_guidance:
                # TERMINAL 3-ENGINE APPROACH:
                # Do not use vertical altitude as a proxy for lateral stopping
                # distance.  The vehicle may be 50 m above the catch plane but
                # have a very different horizontal state than it had 50 m above
                # the ground.  Brake from the actual lateral stopping distance,
                # then remove the remaining lean in the final 5 m above TARGET.
                upright_height = 5.0
                v_terminal = float(seg.v_lat_terminal)
                a_max = max(a_vert * np.tan(seg.max_tilt), 0.05)
                a_small = max(a_vert * np.tan(np.radians(6.0)), 0.05)
                ...
                for j in range(2):
                    ...
                    if h_left <= upright_height:
                        # Final 5 m: thrust vector goes vertical.  A small
                        # residual lateral velocity is allowed at contact.
                        a_lat[j] = 0.0
                        continue
                    ...
                    # Positive distance means the catch point is still ahead
                    # along the current direction of travel.
                    d_ahead = -e * vdir

                    if speed > v_terminal and d_ahead > 0.0:
                        # Physical stopping distance for the current state.
                        d_stop_max = ((speed * speed - v_terminal * v_terminal)
                                      / (2.0 * a_max))
                        d_stop_small = ((speed * speed - v_terminal * v_terminal)
                                        / (2.0 * a_small))

                        if d_stop_small < d_ahead:
                            # There is ample lateral runway.  Keep only a small
                            # intentional lean; do NOT zero vx early.
                            a_req = a_small
                        else:
                            # Braking is now geometrically necessary.  Size the
                            # acceleration from the actual horizontal distance
                            # to the catch point, with max tilt as the hard cap.
                            a_req = ((speed * speed - v_terminal * v_terminal)
                                     / (2.0 * max(d_ahead, 0.5)))
                            a_req = min(a_req, a_max)

                            # If even max tilt cannot stop in the remaining
                            # distance, use all available authority immediately.
                            if d_stop_max >= d_ahead:
                                a_req = a_max
                    elif speed > v_terminal and d_ahead <= 0.0:
                        # Moving through/away from the target: use available
                        # authority to reverse the lateral motion.
                        a_req = a_max
                    elif abs(e) > 0.5:
                        # Below the terminal-speed allowance, correct position
                        # gently rather than demanding zero lateral velocity.
                        a_req = min(0.04 * abs(e), a_small)
                    else:
                        a_req = 0.0

                    a_lat[j] = -vdir * a_req

                # The attitude/thrust-vector command is deliberately tapered to
                # zero only in the last 5 m above the actual catch plane.
                final_blend = float(np.clip(
                    (h_left - upright_height) / 5.0, 0.0, 1.0))
                a_lat *= final_blend
            ...
            tilt_limit = max(seg.max_tilt * taper, np.radians(0.2))
            if tilt > tilt_limit:
